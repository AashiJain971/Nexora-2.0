...
async def store_invoice_data(invoice_details: dict, user_id: str) -> bool:
    ...
    if DEMO_MODE:
        ...
        print(f"Stored invoice in demo mode for user {user_id}")
        return True
...
async def get_historical_financial_data(user_id: str) -> dict:
    ...
    if DEMO_MODE:
        ...
        # For demo, simulate some paid invoices (80% paid for good credit score)
        paid_count = int(total_invoices * 0.8) if total_invoices > 1 else 1
        total_paid = total_amount * 0.8 if total_amount > 0 else 0
        total_pending = total_amount - total_paid
        
        payment_completion_rate = paid_count / total_invoices if total_invoices > 0 else 0.8
