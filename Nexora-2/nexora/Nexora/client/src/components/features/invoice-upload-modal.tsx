...
export function InvoiceUploadModal({
  ...
  const processInvoice = async () => {
    ...
    try {
      ...
      let authToken = token;
      let processResponse = await fetch(
        "https://nexora-2-0-6.onrender.com/process-invoice",
        {
          method: "POST",
          headers: {
      ...
      if (
        (processResponse.status === 401 || processResponse.status === 403) &&
        authToken
      ) {
      ...
      if (!processResponse.ok) {
        ...
        if (processResponse.status === 401 || processResponse.status === 403) {
          ...
          throw new Error(
            `Authentication failed: ${processResponse.statusText}`
          );
        }
        throw new Error(
          `Invoice processing failed: ${processResponse.statusText}`
        );
      }
      ...
      const singleInvoiceResponse = await fetch(
        "https://nexora-2-0-6.onrender.com/calculate-single-invoice-credit-score",
        {
          method: "POST",
          headers: {
      ...
      if (singleInvoiceResponse.ok) {
        ...
        console.log(
          "✅ Single invoice credit score calculated:",
          singleInvoiceCreditScore?.final_weighted_credit_score || "N/A"
        );
      }
            ...
            {isProcessing && (
              <Card className="p-4">
                <div className="space-y-3">
                  ...
                  <div className="text-sm text-muted-foreground">
                    Analyzing document structure, extracting data, and verifying
                    authenticity...
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-6 overflow-auto max-h-[80vh]">
                ...
                <div className="space-y-3 mb-6 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>{analysis.invoiceNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Client:</strong> <span>{analysis.client}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Date:</strong> <span>{analysis.date}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Payment Terms:</strong>{" "}
                    <span>{analysis.paymentTerms}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Industry:</strong> <span>{analysis.industry}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Total Amount:</strong>{" "}
                    <span>{formatCurrency(analysis.totalAmount)}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Currency:</strong> <span>{analysis.currency}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Tax:</strong>{" "}
                    <span>{formatCurrency(analysis.taxAmount || 0)}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Extra Charges:</strong>{" "}
                    <span>{formatCurrency(analysis.extraCharges || 0)}</span>
                  </div>
                  <div className="flex justify-between">
                    <strong>Total Line Items:</strong>{" "}
                    <span>{analysis.totalLineItems}</span>
                  </div>
                </div>

                {/* Line Items */}
                <h5 className="font-semibold text-lg mb-3 text-foreground">
                  Line Items
                </h5>
                <div className="overflow-hidden rounded-lg border border-border shadow-sm">
                  <table className="w-full text-sm">
                    <thead className="bg-gradient-to-r from-teal-100 to-blue-100 dark:from-teal-900 dark:to-blue-900">
                      <tr>
                        <th className="p-3 text-left font-semibold text-gray-800 dark:text-gray-100 border-b border-border">
                          Description
                        </th>
                        <th className="p-3 text-right font-semibold text-gray-800 dark:text-gray-100 border-b border-border">
                          Amount
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {Array.isArray(analysis.lineItems) &&
                        analysis.lineItems.map((item: any, index: number) => (
                          <tr
                            key={index}
                            className="bg-card hover:bg-accent/50 transition-colors duration-150"
                          >
                            <td className="p-3 border-b border-border/30 font-medium text-foreground">
                              {item.description}
                            </td>
                            <td className="p-3 text-right border-b border-border/30 font-semibold text-foreground">
                          ...
                          <div className="flex justify-between items-center">
                            <div>
                              <div className="text-gray-600 dark:text-gray-300 text-sm">
                                This Invoice Credibility Score
                              </div>
                              <div className="font-bold text-green-600 text-2xl">
                                {analysis.individual_credit_score_analysis.final_weighted_credit_score?.toFixed(
                                  1
                                )}
                                /100
                              </div>
                  ...
                  <div className="overflow-hidden rounded-lg border border-border shadow-sm">
                    <div className="bg-gradient-to-r from-teal-100 to-blue-100 dark:from-teal-900 dark:to-blue-900 p-4">
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="text-gray-600 dark:text-gray-300 text-sm">
                            Updated Total Credibility Score
                          </div>
                          <div className="font-bold text-teal-accent text-2xl">
                            {analysis?.total_credit_score_analysis?.final_weighted_credit_score?.toFixed(
                              1
                            )}
                            /100
                          </div>
                          <div className="text-sm font-medium text-gray-800 dark:text-gray-100">
                            {
                              analysis?.total_credit_score_analysis
                                ?.score_category
                            }
                          </div>
                          <div className="text-xs text-muted-foreground mt-1">
                        ...
                        return (
                          <>
                            ...
                            {detailedAnalysis.risk_assessment && (
                              <div>
                                <h6 className="font-medium text-orange-600 mb-1">
                                  Risk Assessment:
                                </h6>
                            ...
                            {detailedAnalysis.creditworthiness_summary?.length >
                              0 && (
                              <div>
                                <h6 className="font-medium text-blue-600 mb-1">
                                  Creditworthiness Summary:
                                </h6>
                                <ul className="list-disc pl-5 space-y-1">
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-gradient-to-r from-teal-100 to-blue-100 dark:from-teal-900 dark:to-blue-900">
                          <tr>
                            ...
                            <th className="p-3 text-center font-semibold text-gray-800 dark:text-gray-100 border-b border-border">
                              Score
                            </th>
                            <th className="p-3 text-center font-semibold text-gray-800 dark:text-gray-100 border-b border-border">
                              Weight %
                        ...
                        <tbody>
                          {Object.entries(
                            analysis.total_credit_score_analysis
                              .factor_breakdown
                          ).map(([factor, details]: [string, any], idx) => (
                            <tr
                              key={idx}
                              className="bg-card hover:bg-accent/50 transition-colors duration-150"
                            >
                ...
                {analysis.total_credit_score_analysis?.recommendations && (
                  <div className="mt-6">
                    ...
                    <div className="space-y-3">
                      ...
                      {analysis.total_credit_score_analysis.recommendations
                        .long_term_improvements?.length > 0 && (
                        <div>
                          <h6 className="font-medium text-blue-600 mb-1">
                            Long-term Improvements:
                          </h6>
